...
abstract class CommandLine {
    ...
    fun run(cargoProject: CargoProject, presentableName: String = command, saveConfiguration: Boolean = true) {
        ...
        val runManager = RunManagerEx.getInstanceEx(cargoProject.project)
        val configuration = createRunConfiguration(runManager, name).apply {
    ...
    private fun CargoProject.localConfigurationName(name: String): String = when {
        project.cargoProjects.allProjects.size > 1 -> "$name [$presentableName]"
        else -> name
    }
}
