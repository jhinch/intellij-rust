...
data class RsQualifiedName private constructor(
    ...
    companion object {
        ...
        private fun childItems(raw: String): List<Item>? {
            ...
            fun toChildItem(type: String, name: String): Item? {
            ...
            result += toChildItem(parts[0], parts[1]) ?: return null
            if (parts.size == 4) {
                result += toChildItem(parts[2], parts[3]) ?: return null
            }
        ...
        private fun parentItem(element: RsQualifiedNamedElement): Item? {
            val parentItem: RsQualifiedNamedElement? = when (element) {
                ...
                is RsNamedFieldDecl -> {
                    val parentStruct = element.parentStruct
                    (parentStruct as? RsEnumVariant)?.parentEnum ?: parentStruct
                }
                else -> return null
            }
        ...
        private fun RsQualifiedNamedElement.toChildItems(): List<Item>? {
            ...
            val type = when (this) {
                ...
                is RsNamedFieldDecl -> {
                    val parentStruct = parentStruct ?: return null
                    if (parentStruct is RsEnumVariant) {
                        val variantName = parentStruct.name ?: return null
                        result += Item(variantName, VARIANT, parentStruct)
                        FIELD
                    } else {
                        STRUCTFIELD
                    }
